(function(globalScope) {
    ...
    P.overflow = function(start, power, meta = 1) {
      ...
      if(number.gte(start)) {
        ...
        number = E(10).iteratedexp(meta, number.iteratedlog(10, meta).div(s).pow(power).mul(s));
      }
      return number
    }
    P.scale = function(s, p, mode, rev = false) {
      s = E(s)
      p = E(p)
      var x = this.clone()
      if(x.gte(s)) {
        if([0, "pow"].includes(mode)) x = rev ? x.div(s).root(p).mul(s) : x.div(s).pow(p).mul(s)
        if([1, "exp"].includes(mode)) x = rev ? x.div(s).max(1).log(p).add(s) : ExpantaNum.pow(p, x.sub(s)).mul(s)
        if([2, "dil"].includes(mode)) {
          let s10 = s.log10()
          x = rev ? E(10).pow(x.log10().div(s10).root(p).mul(s10)) : E(10).pow(x.log10().div(s10).pow(p).mul(s10))
        }
        if([3, "alt_exp"].includes(mode)) x = rev ? x.div(s).max(1).log(p).add(1).mul(s) : ExpantaNum.pow(p, x.div(s).sub(1)).mul(s)
      }
      return x
    }
    ...
    P.non_linear_slog10 = function() {
      let x = this.clone()
      if(x.isNaN() || !x.isFinite()) return x
      if(x.gte(ExpantaNum.TETRATED_MAX_SAFE_INTEGER)) return this.slog(10)
      if(x.eq(0)) return E.ONE.neg()
      let current = this.slog().floor().toNumber()
      let step = 1
      while(step >= Number.EPSILON) {
        if(ExpantaNum.non_linear_10tetr(current).lt(x)) {
          current += step
        } else {
          current -= step
          step /= 2
    ...
    P.mlog = function() {
      ...
      if(number.gte('10^^^^10^10')) {
        ...
        number.array[3][1] -= 1
      } else if(number.gte(ExpantaNum.PENTATED_MAX_SAFE_INTEGER)) {
        number.array[2][1] -= 1
      } else {
    ...
    P.arrow = function(arrows) {
      ...
      if(!arrows.isint() || arrows.lt(ExpantaNum.ZERO)) return function(other) {
        return ExpantaNum.NaN.clone();
      };
      if(arrows.eq(ExpantaNum.ZERO)) return function(other) {
        return t.mul(other);
      };
      if(arrows.eq(ExpantaNum.ONE)) return function(other) {
        return t.pow(other);
      };
    ...
    Q.hyper = function(z) {
      z = new ExpantaNum(z);
      if(z.eq(ExpantaNum.ZERO)) return function(x, y) {
        return new ExpantaNum(y).eq(ExpantaNum.ZERO) ? new ExpantaNum(x) : new ExpantaNum(x).add(ExpantaNum.ONE);
      };
      if(z.eq(ExpantaNum.ONE)) return function(x, y) {
    ...
    P.expansion = function(other) {
      ...
      if(other.gt(ExpantaNum.MAX_LAYER)) return ExpantaNum.POSITIVE_INFINITY.clone();
    ...
    P.normalize = function() {
      ...
      if(!x.array || !x.array.length) x.array = [
        [0, 0]
      ];
    ...
    P.toString = function() {
      ...
      else s += "J^" + numberToStringWithoutSeparators(this.layer) + " ";
